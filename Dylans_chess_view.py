...
class game():
    ...
    def __init__(self, rows, solution,moves):
        self.rows = rows
        print(self.rows)
    ...
    def draw_board(self):
        curr_col = 'khaki'
        num_x = 0
        for x in range(0,self.rows*self.square_size,
                                                        self.square_size):
            num_y = 0
            

            for y in range(0,self.rows*self.square_size,
                                                        self.square_size):
    ...
    def handlehover(self,event):
        ...
        for i in self.queens:
            ...
            if self.on_queen == False and self.queens[i][0]-(self.square_size/2) <event.x and event.x <self.queens[i][0]+(self.square_size/2) and self.queens[i][1]-(self.square_size/2)<event.y and event.y <self.queens[i][1]+(self.square_size/2):
                ...
                self.on_queen = True
                break
            elif self.queens[i][0]-(self.square_size/2) <event.x and event.x <self.queens[i][0]+(self.square_size/2) and self.queens[i][1]-(self.square_size/2)<event.y and event.y <self.queens[i][1]+(self.square_size/2):
                self.canvas.config(cursor="hand1")
                break
        ...
        if self.on_queen == False:
            # print(f"here{random.randint(0,4)}")
            # print(event)
